function h=m_windbarb(long,lat,u,v,fullbarb,varargin)
...
triangle = fullbarb*5;
%/ full barb
g{1} =  umag >= fullbarb   & umag < triangle;
g{2} =  umag >= fullbarb*2 & umag < triangle;
g{3} =  umag >= fullbarb*3;
g{4} = (umag >= fullbarb*4 & umag < triangle) | (umag >= triangle+fullbarb & umag < triangle*2);
g{5} =  umag >= fullbarb*7;
g{6} = (umag >= fullbarb*8 & umag < triangle*2)  | (umag >= triangle*2 + fullbarb);
g{7} = (umag >= fullbarb*9 & umag < triangle*2)  | (umag >= triangle*2 + fullbarb*2);
g{8} =  umag >= triangle*2 + fullbarb*3;
%/ half barb
g{9} = (umag >= fullbarb*0.5 & umag < fullbarb) | (umag >= fullbarb*1.5 & umag < fullbarb*2); %/ h1=h2
g{10} = umag >= fullbarb*2.5 & umag < fullbarb*3; %/h3
g{11} =(umag >= fullbarb*3.5 & umag < fullbarb*4) | (umag >= triangle+fullbarb*0.5 & umag < triangle+fullbarb);%/h4=h6
g{12} =(umag >= fullbarb*4.5 & umag < triangle) | (umag >= triangle+fullbarb*1.5 & umag < triangle+fullbarb*2); %/ h5=h7

g{13} =(umag >= fullbarb*7.5 & umag < triangle+fullbarb*3) | (umag >= triangle*2+fullbarb*0.5 & umag < triangle*2+fullbarb); %/h8=h11
g{14} =(umag >= fullbarb*8.5 & umag < triangle+fullbarb*4) | (umag >= triangle*2+fullbarb*1.5 & umag < triangle*2+fullbarb*2); %/h9=h12
g{15} =(umag >= fullbarb*9.5 & umag < triangle*2)          | (umag >= triangle*2+fullbarb*2.5 & umag < triangle*2+fullbarb*3); %/h10=h13
g{16} = umag >= triangle;
g{17} = umag >= triangle*2;
g{18} = umag>0;    % All have background line
